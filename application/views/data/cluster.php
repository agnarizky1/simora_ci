...
<script>
    // CARA NAMPILKAN MAP
    var mymap = L.map('mapid').setView([-7.98173, 112.6301763], 24);
    ...
    <?php foreach ($cellplan as $cp) { ?>
        var lokasi = L.marker([<?= $cp['latitude'] ?>, <?= $cp['longitude'] ?>], {

        }).bindPopup("<b>Provider : <?= $cp['nama_perusahaan'] ?></b><br> Detail Pemilik : <?= $cp['detail_pemilik'] ?><br><?= $cp['lokasi'] ?><br> Latitude : <?= $cp['latitude']  ?><br> Longitude : <?= $cp['longitude'] ?>")
